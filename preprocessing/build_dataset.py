...
def load_golden_sample_metadata(fits_path):
    ...
    with fits.open(fits_path) as hdul:
        ...
        df = pd.DataFrame({
            'RESOL': filtered_data['RESOL'],
            'SNR_blue': filtered_data['SNR_blue_QAG'],
            'SNR_green': filtered_data['SNR_green_QAG'],
            'SNR_red': filtered_data['SNR_red_QAG']
        })
...
def process_spectrum_data(args):
    ...
    try:
        ...
        # Apply masking if SNR is NaN
        if np.isnan(snr_blue):
            final_flux[mask_blue] = 0
        if np.isnan(snr_green):
        ...
        # Normalize Blue+Green
        if np.any(mask_bg) and np.any(final_flux[mask_bg] > 0):
            flux_bg = final_flux[mask_bg]
            wave_bg = wave_grid[mask_bg]
            norm_flux_bg, continuum_bg = cont_norm.robust_polyfit_huber(flux_bg, wave_bg)
            final_flux[mask_bg] = norm_flux_bg
            
        # Normalize Red
        if np.any(mask_r) and np.any(final_flux[mask_r] > 0):
            flux_r = final_flux[mask_r]
            wave_r = wave_grid[mask_r]
            norm_flux_r, continuum_r = cont_norm.robust_polyfit_huber(flux_r, wave_r)
            final_flux[mask_r] = norm_flux_r
