...
class SpectralDataset(Dataset):
    ...
    *transform and target_transform can be used to augment and/or preprocess the data
    ...
    def __init__(
        ...
        target_key: str = 'labels', # Changed default to 'labels' to match preprocessing
        ...
        self.targets = None # Store targets in memory
    ...
    def _load_metadata(self):
        ...
        with h5py.File(self.hdf5_filepath, 'r') as f:
            ...
            if self.load_targets:
                ...
                # Check if target_key is a group (new format) or dataset (old format)
                if isinstance(f[self.target_key], h5py.Group):
                    self.is_target_group = True
                    if self.target_cols is None:
                        # If no columns specified, use all keys in group (sorted)
                        # Filter out non-numeric datasets
                        all_keys = sorted(list(f[self.target_key].keys()))
                        self.target_cols = []
                        grp = f[self.target_key]
                        for k in all_keys:
                            ds = grp[k]
                            # Check if numeric (int, uint, float)
                    ...
                    for col in self.target_cols:
                        targets_list.append(grp[col][:])
                    self.targets = np.stack(targets_list, axis=1) # (N, n_targets)
                    
                else:
                    self.is_target_group = False
                    # Determine the number of targets based on the user's selection
                    if self.use_target_indices:
                        self.n_targets = len(self.use_target_indices)
                        self.targets = f[self.target_key][:, self.use_target_indices]
                    else:
